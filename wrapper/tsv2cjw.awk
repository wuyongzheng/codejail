...
#		print "type:\t" type;
	...
	print "\tif (cj_get_state() != CJS_MAIN) {";
	if ($1 == "void") {
		printf("\t\trealfunc(%s);\n", implode(arrn, ", "));
		print "\t\treturn;"
	} else {
		printf("\t\treturn realfunc(%s);\n", implode(arrn, ", "));
	}
	print "\t}";
	print "";

	printf("\tfprintf(stderr, \"Jailing " $2 "(%s)\\n\"%s);\n",
		implode(arrf, ", "),
		NF == 2 ? "" : ", " implode(arrn, ", "));
	if ($1 == "void") {
		printf("\tcj_jail(realfunc, %d%s%s);\n", NF-2, NF>2 ? ", " : "", implode(arrn, ", "));
	} else {
		printf("\t%s retval = (%s)cj_jail(realfunc, %d%s%s);\n", $1, $1, NF-2, NF>2 ? ", " : "", implode(arrn, ", "));
		print "";
		print "\treturn retval;";
	}
